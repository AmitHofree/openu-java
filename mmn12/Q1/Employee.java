...
public abstract class Employee {
    ...
    protected abstract double baseEarnings(); // no implementation here

    public boolean isBirthdayMonth() {
        int currentMonth = Calendar.getInstance().get(Calendar.MONTH) + 1   ;
        int birthDayMonth = getBirthDate().getMonth();
        return currentMonth == birthDayMonth;
    }

    public double earnings() {
        double baseEarnings = baseEarnings();
        if (isBirthdayMonth()) {
            return baseEarnings + 200;
        } else {
            return baseEarnings;
        }
    }
